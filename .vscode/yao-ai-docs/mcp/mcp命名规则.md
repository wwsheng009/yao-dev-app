...
| 传输时的名称       | server_id 中的所有 `.` 都会被替换成 `_`<br>然后再拼接 `__{tool_name}` | `github.enterprise` → `github_enterprise__search` |
| 最终呈现给 LLM 的名称 | `{server_id中.变_}__{tool_name}`                                     | `github_enterprise__search`<br>`agent_v2__execute` |
| LLM 回调回来时解析 | 1. 以最后一个 `__` 为分界，右边为 tool_name<br>2. 左边部分所有 `_` 还原成 `.`<br>3. 得到原始 server_id | `github_enterprise__search` → `github.enterprise` + `search` |
